...
export const middleware = (operationId: string, operations: Map<string, ValidateFunction>, exclude: boolean, operationObject?: OpenAPIV3.OperationObject): RequestHandler => {
  const openApiPathMiddleware = (req: Request, res: Response, next: NextFunction) => {
    ...
    const validate: ValidateFunction | undefined = operations.get(operationId);
    if (!validate) {
      return next();
    }
    if (!validate(req)) {
  ...
  openApiPathMiddleware.bind(this);
