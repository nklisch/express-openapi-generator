...
export default class ExpressPathParser {
    ...
    /**
     * Rerusive traversesal methodfor the express router and middleware tree.
    ...
    /**
         ...
         * @param basePath The base path as it was initial delcared for this route
         * @returns 
         */
    parseRouteLayer = (layer: Layer, keys: Key[], basePath: string): ExpressPath => {
        ...
        if (filtered.length === 1) {
            ...
            throw Error(`At most one OpenApiPathMiddleware may be on a route: ${basePath + layer.route.path} has ${filtered.length}`)
        }
        if (openApiDoc) {
            return { path: basePath + layer.route.path, pathParams: params, method: lastRequestHandler.method, openApiOperation: openApiDoc }
        }
 ...
 * @param layerRegexPath The regex for a react router with path pararmters
