...
import express, { Request, RequestHandler, Response, Express, Router } from 'express';
import ExpressPathParser, { onlyForTesting } from '../express-parser';
import { ExpressRegex } from '../types';
import OpenApiPath from '../express-open-api-middleware/index'
...
const pathDoc: OpenAPIV3.OperationObject = {
...
describe('it parses an express app with', () => {
    let app: Express;
    const successResponse: RequestHandler = (req: Request, res: Response) => { res.status(204).send() }
    beforeEach(() => {
        app = express();
    })
    ...
    it('route pattern', (done) => {
        app.route('/test').all((req, res, next) => next()).get(successResponse);
        const parsed = new ExpressPathParser(app);
        const { path, method, pathParams } = parsed.appPaths[0];
    ...
    it('an openApiPath middleware path doc extraction', (done) => {
        const oapi = new OpenApiPath();
        app.get('/test', oapi.path('test', { pathDoc }), successResponse)
        const parsed = new ExpressPathParser(app);
        const { path, method, pathParams, openApiOperation } = parsed.appPaths[0];
        expect(path).toBe('/test')
        expect(method).toBe('get')
        expect(pathParams).toEqual([])
        expect(openApiOperation).toEqual(pathDoc)
        done();
    });

    it('too many openApiPath middlewares on a route should fail', (done) => {
        const oapi = new OpenApiPath();
        app.get('/test', oapi.path('test', { pathDoc }), oapi.path('test', { pathDoc }), successResponse)
    ...
    it('openApiPathMiddleware is on a app use function should fail', (done) => {
        const oapi = new OpenApiPath();
        app.use(oapi.path('test', { pathDoc }))
        app.get('/test', oapi.path('test', { pathDoc }), successResponse)
        expect(() => new ExpressPathParser(app)).toThrow();
        done()
    });
...
describe('parses an express app with ', () => {
    ...
    it('sub-routes with openApiMiddleware', (done) => {
        const oapi = new OpenApiPath();
        subrouter.get('/endpoint', oapi.path('test', { pathDoc }), successResponse);
        router.use('/sub-route', subrouter);
        app.use('/test', router);
        const parsed = new ExpressPathParser(app);
        const { path, method, pathParams, openApiOperation } = parsed.appPaths[0];
