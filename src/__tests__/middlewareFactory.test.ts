...
test('selectOperation selects correct operation', () => {
  expect(onlyForTesting.selectOperation('getUserByName', swaggerEx.paths as OpenAPIV3.PathsObject)).toEqual(swaggerEx.paths['/2.0/users/{username}'].get)
})

test('selectOperation returns undefined ', () => {
  expect(onlyForTesting.selectOperation('testingUndefined', swaggerEx.paths as OpenAPIV3.PathsObject)).toEqual(undefined)
})
...
test('path middleware is correctly created', () => {
  const path = PathMiddleware.path('test', { operationObject: swaggerEx.paths['/2.0/users/{username}'].get as OpenAPIV3.OperationObject })
  ...
  path({} as Request, {} as Response, () => { return; })
