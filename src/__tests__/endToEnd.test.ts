...
describe('it parses an Express application and ', () => {
    ...
    describe('creates an open api document', () => {
        ...
        test('with attached open api documentation', () => {
            app.get(
                '/test/:testing',
                PathMiddleware.path('test', { operationObject: simpleOperation }),
                successResponse,
            );
            documentBuilder.generatePathsObject(app);
            expect(validator.validate(documentBuilder.build()).errors.length).toEqual(0);
        ...
        test('with references for parameters', () => {
            ...
            documentBuilder.parameter('testing', {
                component: { name: 'testing', in: 'path', schema: { type: 'integer' }, required: true },
            });
            operation.parameters = [
                documentBuilder.parameter('year', { copy: true }) as OpenAPIV3.ParameterObject,
                documentBuilder.parameter('testing') as OpenAPIV3.ReferenceObject,
            ];
            ...
            expect(documentBuilder.build()).toEqual({
                ...
                components: {
                    parameters: {
                        ...
                        testing: { name: 'testing', in: 'path', schema: { type: 'integer' }, required: true },
                    },
                },
            });
        });
        ...
        test('with excluded path', () => {
            app.get(
                '/test/:testing',
                PathMiddleware.path('test', { operationObject: simpleOperation, exclude: true }),
                successResponse,
            );
            documentBuilder.generatePathsObject(app);
            expect(validator.validate(documentBuilder.build()).errors.length).toEqual(0);
